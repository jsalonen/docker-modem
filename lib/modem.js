  ...
  req.on('response', function(res) {
    if (context.isStream === true) {
      ...
      res.on('end', function() {
        ...
        if((res.headers['content-type'] || '').toLowerCase() === 'application/json') {
          try {
            payload = JSON.parse(buffer);
          } catch (error) {
            self.buildPayload("JSON parse error", context.isStream, context.statusCodes, false, req, res, null, callback);
            return;
          }
        } else {
        ...
        self.buildPayload(null, context.isStream, context.statusCodes, false, req, res, payload, callback);
      });
    }
  });
